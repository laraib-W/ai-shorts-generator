...
def _sanitize_upload_filename(filename: str, request_id: str) -> str:
    # Browsers or clients may include directory info or even path traversal
    # segments like ../. Keep only the bare filename to prevent the upload
    # endpoint from writing files outside the target directory.
...
def _task_file_to_uri(file: str, endpoint: str, task_dir: str, request_id: str) -> str:
    ...
    try:
        ...
        # Task state should only contain artifact paths within the task directory.
        # Do not wrap the abnormal path into a URL; keep the original value for
        # debugging historical dirty data.
        logger.warning(
        ...
        bgm_list.append(
            {
                ...
                # Only return the filename to avoid exposing server absolute paths
                # to callers. The server will resolve it back to the songs allowlist dir.
                "file": filename,
            }
        )
...
def get_video_materials_list(request: Request):
    ...
    # Filesystem enumeration order is unstable; returning as-is would make
    # "sequential concat" inconsistent across machines or runs. Sort by filename
    # to ensure predictable order from the server.
        ...
        video_materials_list.append(
            {
                "name": filename,
                "size": os.path.getsize(file),
                # Like BGM, only return the filename; resolve within the
                # local_videos allowlist directory when creating a task, to
                # avoid exposing host absolute paths via the API.
                "file": filename,
            }
        )
...
def upload_video_material_file(request: Request, file: UploadFile = File(...)):
    ...
    # Validate against lowercase extensions to accept uppercase suffixes like .MOV.
